...
\section{Structure blende: coordinence 4\---4}\index{blende|(}\index{ZnS|(}
\subsection{Description de la structure}
...
\end{equation}
\begin{rem}
    Pour le sulfure de zinc le rapport des rayons ioniques donne\index{ZnS}\index{blende}
...
\begin{rem}
    Pour CsCl\index{CsCl}, le rapport $x = \np{0,92}$ et les résultats 
    expérimentaux sont 
    compatibles avec le modèle. Il en est de même pour CsBr \index{CsBr}
    ($x = \np{0,86}$) et pour CsI ($x = \np{0,77}$).
\end{rem}\index{CsBr}\index{CsI}\index{CsCl!$x$}
\begin{rem}
    \`A température supérieure à 469\degre C, le chlorure de césium \index{CsCl}
    prend la structure du NaCl (coodinence 6\---6).
\end{rem}



\section{Structure fluorine: coordinence 8\--- 4}\index{fluorine|(}
La structure fluorine est adoptée par le fluorure de calcium ou fluorine de formule
brute CaF$_2$\index{CaF$_2$}. Elle se rencontre pour d'autres fluorures métalliques comme
...
\end{equation}
Pour le fluorure de calcium\index{CaF$_2$}, $r_+ = 94$pm, $r_- = 133$pm :
$x = \np{0,74}$\index{fluorine|)}


\section{Propriétés physiques}
Les températures de fusion sont donc élevées (801\degre C pour NaCl)
\index{NaCl!température de fusion} \index{température de fusion!NaCl}.
